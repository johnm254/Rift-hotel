...
export default function RoomDetail() {
  ...
  const { data: room, isLoading } = useQuery({
    queryKey: ['room', id],
    queryFn: () => api.get(`/rooms/${id}`).then(r => r.data).catch(() => mockRooms.find(r => r.id === id) || null),
  });

  // Reviews — try API, fall back to mock
  const { data: reviews = [] } = useQuery({
    queryKey: ['reviews', id],
    queryFn: () => api.get(`/rooms/${id}/reviews`).then(r => r.data).catch(() => mockReviews.filter(r => r.roomId === id)),
    enabled: !!id,
  });
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div>
              <h2 className="text-2xl font-serif font-bold text-navy mb-6">
                Guest Reviews
                {avgRating && (
                  <span className="ml-3 text-lg font-normal text-gold">★ {avgRating} <span className="text-muted text-sm">({reviews.length})</span></span>
                )}
              </h2>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted">{new Date(review.createdAt).toLocaleDateString('en-KE', { year: 'numeric', month: 'short', day: 'numeric' })}</div>
                          </div>
                        </div>
