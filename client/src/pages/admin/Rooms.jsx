...
export default function AdminRooms() {
  ...
  const saveRoom = useMutation({
    mutationFn: (formData) => {
      if (editing) {
        return api.put(`/rooms/${editing}`, formData, {
    ...
    onError: (err) => {
      setMessage('Error: ' + (err.response?.data?.error || 'Failed to save room'));
    }
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
          <h1 className="text-3xl font-serif font-bold text-navy">Room Management</h1>
          <p className="text-muted text-sm mt-1">{rooms?.length || 0} rooms total</p>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl border border-cream-dark p-6 mb-8 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.amenities}
              ...
              className="w-full px-4 py-3 rounded-xl bg-cream border border-cream-dark focus:border-gold focus:outline-none text-navy"
              placeholder="WiFi, TV, AC, Mini Bar, Ocean View" />
          </div>
          <div>
            <label className="block text-sm font-medium text-navy mb-1">Photos</label>
      ...
      <div className="space-y-4">
        {rooms?.map(room => (
          <div key={room.id} className="bg-white rounded-2xl shadow-md border border-cream-dark p-5">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="w-full md:w-40 h-28 rounded-xl overflow-hidden shrink-0">
                <img src={room.photos?.[0] || 'https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=200'} alt={room.name}
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-muted">
                  <span>🛏 {room.capacity} guests</span>
                  <span className={`px-2 py-1 rounded-full font-medium ${room.available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {room.available ? 'Available' : 'Unavailable'}
                  </span>
                  <span>{room.photos?.length || 0} photos</span>
                  <span>{room.amenities?.length || 0} amenities</span>
              ...
              <div className="flex gap-2 items-start">
                <button onClick={() => startEdit(room)}
                  className="bg-cream hover:bg-cream-dark text-navy px-4 py-2 rounded-xl text-sm font-medium transition-colors">
                  Edit
                </button>
                <button onClick={() => { if (confirm('Delete this room?')) deleteRoom.mutate(room.id); }}
                  className="bg-red-50 hover:bg-red-100 text-red-700 px-4 py-2 rounded-xl text-sm font-medium transition-colors">
                  Delete
                </button>
              </div>
            </div>
        ...
        {rooms?.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl border border-cream-dark">
            <div className="text-4xl mb-3">🛏</div>
            <p className="text-muted">No rooms yet. Add your first room above.</p>
          </div>
        )}
      </div>
    </div>
